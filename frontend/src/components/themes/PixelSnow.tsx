...
void main() {
  ...
  vec2 centeredCoord = fragCoord - res * 0.5;
  vec3 ray = normalize(vec3(centeredCoord * invResX, 1.0));
  ...
  for (int i = 0; i < 128; i++) {
    ...
    if (cellHash < uDensity) {
      ...
      if (toIntersection > 0.0) {
        ...
        float currentFlakeSize = max(uFlakeSize, uMinFlakeSize * depth * halfInvResX);
        ...
        if (uVariant < 0.5) {
          ...
          float invFlakeSize = 1.0 / currentFlakeSize;
          dist = snowflakeDist(vec2(testX, testY) * invFlakeSize) * currentFlakeSize;
        }

        if (dist < currentFlakeSize) {
          float flakeSizeRatio = uFlakeSize / currentFlakeSize;
...
export default function PixelSnow({
  ...
  return <div ref={containerRef} className={`pixel-snow-container ${className}`} style={{ ...style, position: 'fixed', inset: 0, pointerEvents: 'none' }} />;
}
